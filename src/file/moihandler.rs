...
use crate::database::varcharstructure::VarCharStructure;
...
pub fn load_moi_file(mtd: &MtdFile) -> Result<Table, Error> {
    ...
    match res {
        ...
        Err(_) => {
            error!("Unable to write the id to a moi file")
        }
    }
    ...
    for linecounter in 0..number_of_lines {
            ...
            match column_datatype {
                ...
                _ => {
                    return Err(Error::new(
                        ...
                        "Unable to parse data from moi file",
                    ));
                }
            }
        }
        ...
        let save_row: IndexValue = IndexValue::Row(datarow.clone());
        let data_id: RowId = row_id as RowId;
        table.data.insert(save_row, data_id);

        // add data to index structure
        for columnindex in 0..datarow.data.len() {
            let column_type = &datarow.data[columnindex];
            match column_type {
                DataType::BigInt(biggy) => {
                    let boxed = &mut table.index_structures[columnindex];
                    let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                    tree.insert(IndexValue::BigInt(*biggy), data_id);
                }
                DataType::Int(inty) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Int(*inty), data_id);
                }
                DataType::SmallInt(small) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::SmallInt(*small), data_id);
                }
                DataType::TinyInt(tiny) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::TinyInt(*tiny), data_id);
                }
                DataType::Decimal(deci) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Decimal(*deci), data_id);
                }
                DataType::Float(floaty) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Float(*floaty), data_id);
                }
                DataType::VarChar(len, text) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Text(text.clone()), data_id);
                }
                DataType::Bool(booly) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Bool(*booly), data_id);
                }
                DataType::Date(date) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Date(*date), data_id);
                }
                DataType::Time(time) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Date(*time), data_id);
                }
                DataType::DateTime(dt) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Date(*dt), data_id);
                }
                DataType::Enum(act, _pos) => {
                        let boxed = &mut table.index_structures[columnindex];
                        let mut tree: &mut dyn MemoryStructure = &mut **boxed;
                        tree.insert(IndexValue::Enum(act.clone()), data_id);
                }
                _ =>{}
            }
        }
...
fn create_index_structures(table: &mut Table) -> &mut Table {
    ...
    for columntype in columntypes.iter() {
        match columntype {
            DataType::BigInt(x) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<i64>::default()));
            }
            DataType::Int(_) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<i32>::default()));
            }
            DataType::SmallInt(_) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<i16>::default()));
            }
            DataType::TinyInt(_) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<i8>::default()));
            }
            DataType::Decimal(_) => {
                indexstructures.insert(counter, Box::new(ListStructure::<f32>::default()));
            }
            DataType::Float(_) => {
                indexstructures.insert(counter, Box::new(ListStructure::<f64>::default()));
            }
            DataType::VarChar(_, _) => {
                indexstructures.insert(counter, Box::new(VarCharStructure::default()));
            }
            DataType::Bool(_) => {
                indexstructures.insert(counter, Box::new(BooleanStructure::default()));
            }
            DataType::Date(_) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<u64>::default()));
            }
            DataType::Time(_) => {
                indexstructures.insert(counter, Box::new(BPlusTree::<u64>::default()));
            }
...
pub fn create_moi_file(path: &str) -> anyhow::Result<()> {
    ...
    match file_result {
        Ok(file_item) => {
            ...
            let max_id: u64 = 3;
            writer
                .write(&max_id.to_le_bytes())
                .expect("unable to write to disc");
            writer.write(b"\n").expect("unable to write to disc");

            let lines: u64 = 3;
            writer
                .write(&lines.to_le_bytes())
                .expect("unable to write to disc");
            writer.write(b"\n").expect("unable to write to disc");
            return Ok(());
        }
    ...
    match pos_lines {
        Ok(_) => {
            ...
            match res {
                Ok(_) => {}
                Err(_) => {
                    error!("Unable to write a number of lines to a moi file")
                }
            }
        }
    ...
    for i in 0..row.data.len() {
        ...
        match cell {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a BigInt to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                        error!("Unable to get the correct position in a moi file")
                    }
                }
            }
            DataType::Int(number) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a Int to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a SmallInt to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a TinyInt to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a Decimal to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a Float to a moi file")
                            }
                        }
                    }
                    Err(_) => {
            ...
            DataType::VarChar(_len, text) => {
                ...
                match pos {
                    Ok(_) => {
                        file.write_all(&number_of_chars.to_le_bytes())
                            .expect("unable to write to disc");
                        let pos = file.seek(SeekFrom::End(0));
                        match pos {
                            Ok(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a Bool to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a date to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a Time to a moi file")
                            }
                        }
                    }
                    Err(_) => {
                ...
                match pos {
                    Ok(_) => {
                        ...
                        match res {
                            Ok(_) => {}
                            Err(_) => {
                                error!("Unable to write a DateTime to a moi file")
                            }
                        }
                    }
                    Err(_) => {
            ...
            DataType::Null => {
                ...
                match pos {
                    Ok(_) => {
                        file.write_all((&nul).as_ref())
                            .expect("unable to write NULL value");
                    }
                    Err(_) => {
                        error!("Unable to get the correct position in a moi file")
                    }
...
mod tests {
        ...
        match file_result {
            Ok(file_item) => {
                ...
                writer
                    ...
                    .expect("unable to write to disc");
                writer
                    .write_all((&type_1).as_ref())
                    .expect("unable to write to disc");

                writer.write(b"\n").expect("unable to write to disc");

                writer
                    .write(&id_2.to_le_bytes())
                    .expect("unable to write to disc");
            ...
            Err(error) => {
                println!(
                    "Something went terribly wrong reading user table: {}",
                    error
                );
            }
        }
    ...
    fn create_system_tables_moi() {
        ...
        match file_result {
            Ok(file_item) => {
                ...
                while counter < lines {
                    ...
                    if id == 1 {
                        ...
                        error!(
                            "There's something strange in your neighbourhood ... who do you gonna call"
                        );
                    }
            ...
            Err(error) => {
                error!(
                    "Something went terribly wrong reading system tables table: {}",
                    error
                );
            }
        }
    ...
    fn create_system_database_moi() {
        ...
        match file_result {
            ...
            Err(error) => {
                error!(
                    "Something went terribly wrong reading system database tables: {}",
                    error
                );
            }
        }
